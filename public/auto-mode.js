    ...
    try {
      ...
      if (badge) {
        ...
        badge.innerHTML = periods.map(d =>
          '<div class="settings-stat"><div class="settings-stat-label">' + esc(d.label) +
          '</div><div class="settings-stat-val">$' + fmt(d.entries) +
          '</div><div class="settings-stat-sub">' + tok(d.entries) + ' Token</div></div>'
        ).join('');
      }
    } catch (err) {
