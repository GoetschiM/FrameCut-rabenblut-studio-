...
(() => {
  ...
  async function autoPlan() {
    ...
    let savedStyles=[];
    try { savedStyles=(await api('/api/style-presets')).presets||[]; } catch (err) { notice(`Stilbibliothek konnte nicht geladen werden: ${err.message}`); }
    const savedStyleOptions=savedStyles.map(item=>`<option value="saved:${item.id}">${esc(item.name)}</option>`).join('');
