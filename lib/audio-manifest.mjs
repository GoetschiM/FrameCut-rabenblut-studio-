...
function sourceRevision(shots, dialogue) {
  const source = { shots: shots.map(s => [s.id, s.sequence, s.duration_seconds]), dialogue: dialogue.map(d => [d.id, d.shot_id, d.sequence, d.asset_id, d.text, d.voice || '']) };
...
export function createEpisodeAudioManifest({ ownerId, projectId, episodeId, shots, dialogue }) {
  let cursor = 0;
  ...
  return {
    ...
    source_revision: sourceRevision(shots, dialogue),
